...
class FrozenLakeQLearningAgent:
    def __init__(
        self,
        state_space: int,
        action_space: int,
        ...
        epsilon_decay: float = 0.995,  # Slightly faster decay for shorter training
    ):
...
def run_single_trial(trial_num: int, episodes: int = 1000) -> Tuple[List[float], List[float], List[float]]:
    """Run a single training trial with improved rewards"""
...
def run_multiple_trials(num_trials: int = 10, episodes: int = 1000) -> Tuple[List[List[float]], List[List[float]], List[List[float]]]:
    ...
    print(f"\nStarting training for {num_trials} trials, {episodes} episodes each...")
...
def plot_multi_trial_metrics(all_rewards: List[List[float]], all_steps: List[List[float]], 
    ...
    for ax, data, title, ylabel in zip(
        ...
        ax.plot(x, mean, label='Mean across 10 trials', color='blue', linewidth=2)
    ...
    plt.suptitle('FrozenLake Q-Learning Performance over 10 Trials', fontsize=14)
    plt.tight_layout()
    plt.show()
...
def print_trial_statistics(all_rewards: List[List[float]], all_steps: List[List[float]], 
    ...
    final_rewards = [np.mean(rewards[-50:]) for rewards in all_rewards]  # Last 50 episodes
